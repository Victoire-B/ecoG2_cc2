...
```{r}
path <- "~/ecog2_cc2/Seqreunies2" # CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
```
...
- truncLen=c : prend les deux (forward et reverse) et on coupe au niveau respectif de 24à pb et 200pb environ au niveua ou le score quality est en dessous de Q30
